...
$.fn.pageList = function (arg, methodName) {
    if (typeof methodName === "string" && $.trim(methodName) != "") {  // trim(参数, 【str】); 去除参数
        ...
        var totalPages = ~~(arg.totalCount / arg.pageSize) + (~~(arg.totalCount % arg.pageSize) == 0 ? 0 : 1);//获取总页数 C/C++ 语言中 ~ 是一种 “运算符”-- “二进制 按位 否 运算符”。例如  int x = 0x79;   写成二进制  0111 1001； ~x  运算 结果 是 1000 0110;也就是 某位是 1，就变 0， 是 0， 就变 1。
...
function toolp(){
	$(".cate_list").empty();
	if(num==0){	
		$.post("../Goods.do",{"opr":"GoodsG_g_priceSonDown","g_sonID":1},function(data){
			pagedata=data;
			pages(indexs);
			show();
		},"json")
		...
		$.post("../Goods.do",{"opr":"GoodsG_g_priceSonUp","g_sonID":1},function(data){
			pagedata=data;
			pages(indexs);
			show();
		},"json")
