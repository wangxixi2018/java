...
<div class="soubg">
	<div class="sou">
        ...
        <span class="fr">
        	...
        	<input  id="opol" type="hidden" name="opr" value="${users.u_id}" />
        	<span class="fl">
        	<c:if test="${not empty users}">
        		<c:if test="${not empty users.u_nc}">
        			欢迎您:<a href="${pageContext.request.contextPath}/user/user.jsp" id="u_nc">${users.u_nc}</a>
        		</c:if>
        	...
        	<c:if test="${empty users}">
        		你好，请<a id="index99" href="Login.html">登录</a>&nbsp; <a href="Regist.html" style="color:#ff4e00;">免费注册</a>
        	</c:if>
        	&nbsp;|&nbsp;<a href="Index.jsp">首页</a>&nbsp;|&nbsp;<a href="#">我的订单</a>&nbsp;|&nbsp;<a href="${pageContext.request.contextPath}/user/user.jsp">个人中心</a>&nbsp;|
        	</span>
        	<span class="ss">
            	<div class="ss_list">
                	<a href="#">收藏夹</a>
...
<div class="i_bg">
    ...
    <div class="content">
        ...
        <div id="tsShopContainer">           
        	<c:forEach items="${sessionScope.listProduct}" var="Goods">
            	<div id="tsImgS" class="imgInfo small_pic">
            		<a href="downFile.do?opr=${Goods.g_ID}" title="Images" class="MagicZoom toop" id="MagicZoom"><img class="toops" src="downFile.do?opr=${Goods.g_ID}" width="390" height="390" /></a>
            	</div>
            </c:forEach>
            <div id="tsPicContainer">
                ...
                <div id="tsImgSCon">
                    <ul>
                    	<c:forEach items="${sessionScope.listProduct}" var="Goods" varStatus="num">
                        <li id="img_6" onclick="showPic(0)" rel="MagicZoom" class="tsSelectImg"><img src="downFile.do?opr=${Goods.g_ID}" tsImgS="downFile.do?opr=${Goods.g_ID}" width="79" height="79" /></li>
                        </c:forEach> 
                        <c:forEach items="${sessionScope.listProduct1}" var="Goods" varStatus="num">
                        <li id="img_${num.count}" onclick="showPic(${num.count})" rel="MagicZoom"><img src="downFile.do?opr=${Goods.g_ID}" tsImgS="downFile.do?opr=${Goods.g_ID}" width="79" height="79" /></li>                    
                        </c:forEach>
                    </ul>
                </div>
        ...
        <div class="pro_des">
        	<div class="des_name">
        	 	<c:forEach items="${sessionScope.listG_son}" var="Goods">
            	<p>${Goods.g_describe}</p>
                </c:forEach>
            </div>
            <div class="des_price">
           		 <c:forEach items="${sessionScope.listProduct}" var="Goods">
            	本店价格：<b>${Goods.g_g_price }</b><br />
               	 消费积分：<span>${Goods.g_integral }R</span>
                </c:forEach>
            </div>
            <div class="des_choice">
            	<span class="fl">型号选择：</span>
                <ul>
                	 <c:forEach items="${sessionScope.listG_Size}" var="Goods" varStatus="num">	 	
                		<li onclick="GoodsCol(this,${Goods.g_sizeID})">${Goods.g_size}<div class="ch_img"></div></li> 
                    </c:forEach>
                </ul>
            </div>
            <div class="des_choice">
                ...
                <ul>
                 <c:forEach items="${sessionScope.MAP.map0.listGoods3}" var="Goods0">
                 	 	<input type="hidden" name="Goods0" value="${Goods0.g_ID}" />
                 	 	<c:set var="num0" value="${Goods0.g_ID}"></c:set>
                 </c:forEach> 
                 <c:forEach items="${sessionScope.MAP.map0.listG_Colors}" var="G_Colors0">
                 	 	<li onclick="GoodsColors(${num0},${G_Colors0.g_colorsID},this)">${G_Colors0.g_colors}<div class="ch_img"></div></li>
                 </c:forEach>
                 
                 
                 <c:forEach items="${sessionScope.MAP.map1.listGoods3}" var="Goods1">
                 	 	<input type="hidden" name="Goods1" value="${Goods1.g_ID}" />
                 	 	<c:set var="num1" value="${Goods1.g_ID}"></c:set>
                 </c:forEach>
                  <c:forEach items="${sessionScope.MAP.map1.listG_Colors}" var="G_Colors1">
                 	 	<li onclick="GoodsColors(${num1},${G_Colors1.g_colorsID },this)">${G_Colors1.g_colors}<div class="ch_img"></div></li>
                 </c:forEach>
                 
                 
                 <c:forEach items="${sessionScope.MAP.map2.listGoods3}" var="Goods2">
                 	 	<input type="hidden" name="Goods2" value="${Goods2.g_ID}" />
                 	 	<c:set var="num2" value="${Goods2.g_ID}"></c:set>
                 </c:forEach>
                 <c:forEach items="${sessionScope.MAP.map2.listG_Colors}" var="G_Colors2">
                 	 	<li onclick="GoodsColors(${num2},${G_Colors2.g_colorsID },this)">${G_Colors2.g_colors}<div class="ch_img"></div></li>
                 </c:forEach>
                 
                 
                 
                 <c:forEach items="${sessionScope.MAP.map3.listGoods3}" var="Goods3">
                 	 	<input type="hidden" name="Goods3" value="${Goods3.g_ID}" />
                 	 	<c:set var="num3" value="${Goods3.g_ID}"></c:set>
                 </c:forEach>
                 <c:forEach items="${sessionScope.MAP.map3.listG_Colors}" var="G_Colors3">
                 	 	<li onclick="GoodsColors(${num3},${G_Colors3.g_colorsID },this)">${G_Colors3.g_colors}<div class="ch_img"></div></li>
                 </c:forEach>
                 
                 
                 
                 <c:forEach items="${sessionScope.MAP.map4.listGoods3}" var="Goods4">
                 	 	<input type="hidden" name="Goods4" value="${Goods4.g_ID}" />
                 	 	<c:set var="num4" value="${Goods4.g_ID}"></c:set>
                 </c:forEach>
                 <c:forEach items="${sessionScope.MAP.map4.listG_Colors}" var="G_Colors4">
                 	 	<li onclick="GoodsColors(${num4},${G_Colors4.g_colorsID},this)">${G_Colors4.g_colors}<div class="ch_img"></div></li>
                 </c:forEach>
                 
                 
                 
                 <c:forEach items="${sessionScope.MAP.map5.listGoods3}" var="Goods5">
                 	 	<input type="hidden" name="Goods5" value="${Goods5.g_ID}" />
                 	 	<c:set var="num5" value="${Goods5.g_ID}"></c:set>
                 </c:forEach> 
                 <c:forEach items="${sessionScope.MAP.map5.listG_Colors}" var="G_Colors5">
                 	 	<li onclick="GoodsColors(${num5},${G_Colors5.g_colorsID},this)">${G_Colors5.g_colors}<div class="ch_img"></div></li>
                 </c:forEach>
                </ul>
            </div>
            <div class="des_share">
            ...
            <div class="des_join">
                ...
                <span class="fl"><a onclick="addScar()"><img src="images/j_car.png" /></a></span>
            </div>            
        </div>    
...
<script src="js/ShopShow.js"></script>
	...
	<div class="log_bg89" >	
    	<div class="top89">
       		 <div class="logo89"><a href="Index.html"><img src="${pageContext.request.contextPath}/images/titiyigou_logo.png" width="80px" /></a></div>
   		</div>
		<div class="login89">
	        	<form   method="post" >
	            <table border="0"  cellspacing="0" cellpadding="0">
	              <tr height="50" valign="top89">
	              	<td width="55">登录</td>
	                <td>
	                	<span class="fl89" >&nbsp;</span>
	                    <span class="fr89">还没有商城账号，<a href="Regist.html" style="color:#ff4e00;">立即注册</a></span>
	                </td>
	              </tr>
	              <tr height="50">
	                <td>用户名</td>
	              ...
	              <tr>
	                ...
	                <td >
	                	<span  class="fl89">
	                    	<label class="r_rad89"><input type="checkbox" /></label><label class="r_txt">请保存我这次的登录信息</label>
	                    </span>
	                    <span class="fr89"><a href="#" style="color:#ff4e00;">忘记密码</a></span>
	                </td>
	              </tr>
	              <tr height="60">
	              	<td>&nbsp;</td>
	                <td><input type="button" value="登录" class="log_btn89" onclick="longin()"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id="longin" style="font-size: 16px;color: red;"></span></td>	                
	              </tr>  
	            </table>
	        </form>   
	    </div>
	...
	<%-- <div class="log_bg89" >	
		...
		<div class="login89">
	            ...
	            <table border="0"  cellspacing="0" cellpadding="0">
	              ...
	              <tr height="60">
	              	<td>&nbsp;</td>
	                <td><input type="submit" value="登录" class="log_btn89" /></td>
	              </tr>
	            </table>
	        </form>   
	    </div>
