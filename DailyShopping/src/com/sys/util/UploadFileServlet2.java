...
public class UploadFileServlet2 extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {
					...
					if ("g_ID".equals(name)) {
						...
						G_sort1ID = Integer.parseInt(value.substring(0, value.indexOf(" ")));
					} else if ("G_sort2ID".equals(name)) {
						G_sort2ID = Integer.parseInt(value.substring(0, value.indexOf(" ")));
					} else if ("g_sonID".equals(name)) {
						g_sonID = Integer.parseInt(value.substring(0, value.indexOf(" ")));
					} else if ("g_colorsID".equals(name)) {
						g_colorsID = Integer.parseInt(value.substring(0, value.indexOf(" ")));
					} else if ("g_sizeID".equals(name)) {
						g_sizeID = Integer.parseInt(value.substring(0, value.indexOf(" ")));
						...
						g_rankID = Integer.parseInt(value.substring(0, value.indexOf(" ")));
					}
			...
			System.out.println(goods.getG_colorsID() + "|" + goods.getG_g_price() + "|" + goods.getG_integral() + "|"
					+ goods.getG_rankID() + "|" + goods.getG_sizeID() + "|" + goods.getG_sonID() + "|"
					+ goods.getG_sort1ID() + "|" + goods.getG_sort2ID() + "|" + goods.getG_src());
		...
		/*
		 * String sjon = JSON.toJSONString(g_ID); PrintWriter out =
		 * response.getWriter(); out.write(sjon); out.close();
		 */
		// System.out.println(message); //Goods.do?opr=AddGoods&g_ID=" + g_ID
		request.getRequestDispatcher("Goods.do?opr=AddGoodss&g_ID=" + g_ID).forward(request, response);
	}
